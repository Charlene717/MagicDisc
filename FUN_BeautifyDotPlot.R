BeautifyDotPlot <- function(scRNA.SeuObj, 
                            Path = PathCellType,
  ...
  p1 <- DotPlot(scRNA.SeuObj, features = markers.to.plot, cols = c("lightgrey", "blue"),
  ...
  p2 <- DotPlot(scRNA.SeuObj, features = markers.to.plot, cols = DotPlot_Color1.set,
                dot.scale = 8, split.by = "Sample") + RotatedAxis()
  print(p2)
  
  # https://github.com/satijalab/seurat/issues/1541
  p3 <- DotPlot(scRNA.SeuObj, features = markers.to.plot, cols = DotPlot_Color2.set,
                dot.scale = 8, split.by = "Cachexia") + RotatedAxis()
  print(p3)
  
  p4 <- DotPlot(scRNA.SeuObj, features = markers.to.plot, cols = DotPlot_Color3.set,
